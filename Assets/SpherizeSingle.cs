...
public class SpherizeSingle : MonoBehaviour
{
    ...
    Vector3 SampleHeightMap(Vector2 pointInBounds, Vector4 bounds)
    {
        ...
        if (UnityEngine.Random.Range(0,500) == 250)
        {
        ...
        foreach (var newPlane in faces)
        {
            ...
            if (bend)
            {
                ...
                for (var i = 0; i < vertices.Length; i++)
                {
                    ...
                    /*
                    vertices[i] = newPlane.transform.InverseTransformPoint(
                        newPlane.transform.TransformPoint(vertices[i]).normalized 
                        * (this.radius + 
                        (SampleHeightMap(samplePoint, bounds).z 
                        * heightDamp)) );
                    */

                    //vertices[i] = vertices[i].normalized * UnityEngine.Random.Range(5, 10f);

                    vertices[i] = newPlane.transform.InverseTransformPoint(
                        newPlane.transform.TransformPoint(vertices[i]).normalized
                        * (UnityEngine.Random.Range(5, 10f)));
